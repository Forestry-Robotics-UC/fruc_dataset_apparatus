...
topics=$(kdialog --checklist "Topics to record" \
    ...
    /camera/depth/image_rect_raw "Realsense Depth" off \
    /camera/color/metadata "Realsense Color Metadata" on \
    /camera/depth/metadata "Realsense Depth Metadata" on \
    ...
    /camera/left/image_raw "Realsense IR Left" off \
    /camera/right/image_raw "Realsense IR Right" off \
    /fix "GPS" on)
