...
class LidarPacketHandler {
   ...
   public:
    ...
    bool lidar_handler_sensor_time(const sensor::packet_format&,
        ...
        /* Amended relative counter to use unix timestamp ns offset */
        static uint64_t unix_ns = 0;
        if (!unix_ns) {
            rclcpp::Clock system_clock(RCL_SYSTEM_TIME);
            rclcpp::Time now = system_clock.now();
            unix_ns = static_cast<uint64_t>(now.nanoseconds());
            RCLCPP_INFO(rclcpp::get_logger(getName()), "SET UNIX TIMESTAMP OFFSET");
        }

        if (!(*scan_batcher)(lidar_packet, lidar_scan)) return false;
        lidar_scan_estimated_ts = unix_ns + compute_scan_ts(lidar_scan.timestamp()); // Amended here
